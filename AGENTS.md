...
[docs/venues-plan.md](venues-plan.md) (facility directory — amenities, pricing, venue detail screen) got explicit sign-off 2026-08-15 and A1-A6(P1) are live: schema, `venue_detail`/`venues_near` RPCs, `ui/app/venue/[id].tsx`, and 22 of 31 venues have curated profiles (the CSV's 15 + the NBC/Alpha/BadmintonWorx/The Badminton Club P1 chains). The 51-venue P2 queue and non-chain P1 leads are still unenriched — see venues-plan.md §8. [docs/social-plan.md](social-plan.md) is still proposed, not approved — beyond the MVP spec, needs explicit sign-off before any app code or migration lands.
