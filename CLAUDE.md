...
`ui/.env` is checked in pointing at the local `supabase start` stack — `npm start` / `run:ios` / `run:android` / jest / Maestro e2e all use it, no setup needed. `ui/.env.production` (gitignored) holds the hosted project's URL/key for `eas build`/`expo export` only — real device/store builds always use it, local dev never does. `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY` blank = grey map tiles + no venue search in the wizard. Test login (local only): `test@smashio.dev`, password in `supabase/seed.sql` — do not use a hosted test account credential here, see security-audit-2026-09-11.md H1 (local only, not in the public repo).
