...
- [x] **`lib/sound.ts`** (new) — `expo-audio` (SDK 57) wrapper: preloaded player pool, `sound.play('pop')`, global mute honoring a stored setting, no-op when unsupported. **Blocked on assets** — needs 5 short files in `ui/assets/sfx/`: `pop`, `whoosh`, `chime`, `thunk`, `sparkle`. See [Open decisions](#open-decisions). *(Unblocked — all five `.wav` files exist, synthesised by `ui/scripts/synth-sfx.js` per Decision 1. Tuned later: `359e7dc` softened the whoosh, normalised gains, and added a user-facing mute toggle. Noted 2026-09-07.)*
