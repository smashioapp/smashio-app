...
// Avatars live at {uid}/{filename} in storage; the folder listing only gives back filenames, so
// this rebuilds the full keys the remove() call needs. Pulled out as pure logic so it's
// unit-testable without a storage round trip.
export function avatarPathsFor(userId: string, files: { name: string }[]): string[] {
  return files.map((f) => `${userId}/${f.name}`);
}

// Guarded so `deno test` can import this module for its pure helpers (avatarPathsFor) without
// binding a port — supabase serves this file directly, where import.meta.main is true.
if (import.meta.main) {
