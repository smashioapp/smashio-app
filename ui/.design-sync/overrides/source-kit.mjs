// forked from design-sync lib/source-kit.mjs — this repo ships zero .d.ts
// (no dist build at all), so `exported` (from exportedNames, .d.ts-based) is
// always empty and component inclusion comes entirely from
// `deriveComponentsFromSrc` + `cfg.componentSrcMap`. Upstream only runs the
// derive-from-src fallback when `!components.length` — i.e. AFTER folding in
// componentSrcMap — so a repo that pins even one component (we pin GameMap
// to its .web.tsx variant) ends up with components.length===1 from the pin
// alone, the `!components.length` check is false, and the other ~40
// components never get derived at all. Fix: run the derive-from-src pass
// whenever synth mode has no .d.ts-derived names, BEFORE folding in
// componentSrcMap, so pins always layer on top of full derivation instead of
// suppressing it. Nothing else changed from upstream.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // [OVERRIDE] fork: a componentSrcMap pin only relabels enrichment
    // metadata (step 4) — it doesn't stop the barrel below from ALSO
    // `export *`-ing whatever file the plain name-match heuristic would
    // have picked, which still runs that file's top-level side effects
    // (e.g. GameMap.tsx's react-native-maps codegenNativeComponent calls,
    // even when GameMap.web.tsx is what we actually want bundled). Drop any
    // barrel file that name-matches a pinned component but isn't the
    // pinned file itself.
    const pins = Object.entries(srcMap).filter(([, v]) => typeof v === 'string');
    for (const [name, rel] of pins) {
      const pinnedAbs = slash(resolve(PKG_DIR, rel));
      const kebab = name.replace(/([a-z0-9])([A-Z])/g, '$1-$2');
      const nameRx = new RegExp(
        `(?:^|/)(?:${name}/(?:index|${name})\\.(tsx|jsx)|(?:${name}|${kebab})\\.(tsx|jsx))$`,
        'i',
      );
      comps = comps.filter((p) => slash(resolve(p)) === pinnedAbs || !nameRx.test(p));
    }
  ...
  // [OVERRIDE] fork: derive from src/ whenever there's no .d.ts-derived name
  // to work with, BEFORE folding in componentSrcMap — so a pin (e.g. GameMap
  // -> GameMap.web.tsx) layers on top of full derivation instead of making
  // `names` non-empty and silently skipping derivation for everything else.
  if (!names.size && synthEntry) {
    for (const c of deriveComponentsFromSrc(srcFiles)) names.add(c.name);
  }
